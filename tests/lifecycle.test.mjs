...
test("builds a deterministic self-contained release manifest", () => {
  ...
  assert.equal(first.skill.version, "0.1.1-beta.7");
  ...
  assert.equal(
    first.files.some(
      (file) => file.path === "schemas/decision-bundle.schema.json"
    ),
    true
  );
  assert.equal(
    first.files.some(
      (file) => file.path === "schemas/decision-interaction.schema.json"
    ),
    true
  );
  assert.equal(
  ...
  assert.equal(
    first.files.some(
      (file) => file.path === "schemas/document-maintenance-input.schema.json"
    ),
    true
  );
  assert.equal(
    first.files.some(
      (file) => file.path === "references/business-documentation.md"
    ),
    true
  );
  assert.equal(
...
test("rejects a release channel that disagrees with the semantic version", () => {
  ...
  assert.throws(
    ...
    /channel development does not match version 0\.1\.1-beta\.7/
  );
});
...
test("reports the explicit reinitialization plan for the v2 interface upgrade", () => {
  ...
  assert.deepEqual(
    ...
    ["reinitialize-v2-project-state-from-beta-4-to-beta-7"]
  );
});
